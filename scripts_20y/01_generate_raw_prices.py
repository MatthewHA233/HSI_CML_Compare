"""
并行处理20年港股CSV数据，生成raw_prices.csv（月度数据）
...
- **月度重采样**：取每月最后一个交易日价格（学术研究标准）
- 最后合并成完整的20年月度数据（约240个月，取决于实际数据范围）
"""
import pandas as pd
import numpy as np
...
def process_period_folder(folder_info):
    ...
    # **月度重采样**：取每月最后一个交易日价格
    print(f"[{period_name}] 月度重采样（取月末价格）...")
    print(f"  日度数据: {len(price_pivot.columns)} 个交易日")

    # 转置以便使用resample（需要时间索引在行上）
    price_pivot_T = price_pivot.T
    # 使用resample取每月最后一个有效值
    monthly_prices_T = price_pivot_T.resample('M').last()
    # 转置回来
    price_pivot = monthly_prices_T.T

    print(f"  月度数据: {len(price_pivot.columns)} 个月")
    ...
    month_count = len(price_pivot.columns)

    print(f"[{period_name}] 完成 - {stock_count}只股票 × {month_count}个月 - {processing_time:.1f}秒")

    return output_csv_path, stock_count, month_count, processing_time


def main():
    print("=" * 80)
    print("生成 raw_prices.csv（20年月度数据，2005-2024）")
    print("=" * 80)
    ...
    for folder_name, period_label in period_folders:
        ...
        output_csv_path = project_root / "处理后数据_20y" / "单独csv" / f"{period_label}.csv"
    ...
    # 读取所有时期的CSV
    period_dfs = []
    for csv_path, stock_count, month_count, _ in sorted(results):
        df = pd.read_csv(csv_path)
        print(f"  读取: {csv_path.name} - {stock_count}只股票 × {month_count}个月")
    ...
    print(f"  原月份数: {len(date_cols_all)} 个月")
    print(f"  筛选后: {len(date_cols_filtered)} 个月")
    if len(date_cols_all) != len(date_cols_filtered):
        print(f"  移除2025年数据: {len(date_cols_all) - len(date_cols_filtered)} 个月")

    # 保存最终合并文件
    output_folder = project_root / "处理后数据_20y" / "01_原始价格"
    ...
    print(f"\n生成文件:")
    print(f"  - 单独CSV: {len(results)} 个文件在 处理后数据_20y/单独csv/")
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("港股历史行情数据处理分析报告（20年月度数据：2005-2024）\n")
        f.write("=" * 80 + "\n\n")
        ...
        for i, (csv_path, stock_count, month_count, proc_time) in enumerate(sorted(results), 1):
            f.write(f"{i}. {csv_path.name}\n")
            f.write(f"   股票数量: {stock_count:,} 只\n")
            f.write(f"   月份数: {month_count:,} 个月\n")
        ...
        f.write(f"  总月份数: {len(date_cols):,} 个月\n")
        f.write(f"  信息列数: {len(info_cols)} 列 (Symbol, SecurityID)\n")
        f.write(f"  总列数: {len(merged_df.columns):,} 列\n\n")

        if date_cols:
            f.write(f"时间范围:\n")
            f.write(f"  起始月份: {date_cols[0]}\n")
            f.write(f"  结束月份: {date_cols[-1]}\n")
            f.write(f"  跨度月数: {len(date_cols)} 个月\n\n")
